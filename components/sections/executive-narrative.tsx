...
export function ExecutiveNarrative({ account, competitors }: ExecutiveNarrativeProps) {
    ...
    boardSummary.startStream({
      ...
      body: {
        messages: [
          {
            role: "user",
            content: `Generate a board-level summary for ${account.name}. This should be a 1-page executive brief suitable for internal leadership. Include: account overview, strategic importance, deal status, revenue potential ($${account.estimatedLandValue}M land / $${account.estimatedExpansionValue}M expansion), key risks, competitive dynamics (platform standardization / resilience / AI-adjacent data growth), and what we need from leadership to win. Keep it concise and strategic.`,
          },
        ],
        account,
        competitors,
  ...
  const defaultNarrative = {
    whyNow: "Account is under pressure to reduce outages, harden recovery posture, and modernize the data estate to support growth and AI-adjacent workloads.",
    whyClaude: "Operational outcomes: predictable performance, simpler operations, and a clear land-and-expand path into platform standardization across workloads.",
    whyNot: "Incumbents are entrenched. Win by quantifying outcomes (uptime, recovery time, operational efficiency) and reducing migration/refresh pain.",
    impact: "Platform footprint expansion across Tier-1 workloads, cyber recovery posture, and AI/analytics data paths.",
    governance: "Security and compliance posture, access controls, auditability, and recovery runbooks. Security and Legal review in progress.",
    rollout: "Phase 1: Land a high-pain workload. Phase 2: Expand into adjacent teams. Phase 3: Standardize across the estate.",
  };
  ...
  const sections = [
    ...
    { label: "Why Everpure", content: n.whyClaude },
      ...
      >
        ...
        <div className="pt-3 border-t border-surface-border/40">
          <div className="flex items-center gap-2 mb-2">
            <EverpureLogo size={10} className="text-accent/40" />
